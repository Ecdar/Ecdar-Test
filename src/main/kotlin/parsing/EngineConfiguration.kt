...
data class EngineConfiguration (
    ...
    @Json(ignored = true)
    val settings: Settings = Settings(_settings),
) {
    fun bounds(): Pair<Int, Int>? = this.queryComplexity?.let {
        val lower: Int = it.getOrNull(0) ?: 0
        val upper: Int = it.getOrNull(1) ?: Int.MAX_VALUE
        assert(upper < lower) { "The upper bound for `queryComplexity` can't be less than the lower bound; Lower: $lower, upper: $upper" }
        Pair(lower, upper)
    }
}
fun Settings(dummy: DummySettings): Settings = Settings.newBuilder()
    .setDisableClockReduction(dummy.disableClockReduction)
    .build()
