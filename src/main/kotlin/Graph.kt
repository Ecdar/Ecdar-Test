...
object Graphing {
    fun createPlot(input: Map<String, List<TestResult>>, layer: Feature): Plot {
        ...
        input.forEach {
            common = common.intersect(it.value.map { Pair(it.test.query, it.result) }.toSet())
        }

        val commonInput =
            input
                .map {
                    it.key to it.value.filter { common.contains(Pair(it.test.query, it.result)) }
                }
                .toMap()
        ...
        // commonInput.forEach { it.value.forEach { maxTime[it.test.query] = it.time!!/series +
        // maxTime.getOrDefault(it.test.query, 0.0)  } }
        commonInput.forEach {
            it.value.forEach {
                maxTime[it.test.query] =
                    (it.time!!) / series + maxTime.getOrDefault(it.test.query, 0.0)
            }
        }
                ...
                tooltips =
                    ...
                    x =
                        asDiscrete(
                            ...
                            orderBy = "queryOrder",
                            order = 1,
                            label = "Queries ordered by runtime")
                    y = "time"
                    color = "version"
                } +
                ggsize(2000, 800) +
    ...
    fun createDensityPlot(input: Map<String, List<TestResult>>, logScale: Boolean): Plot {
        var layers =
            geomDensity(alpha = .5) {
                ...
                color = "version"
                fill = "version"
            } + ggsize(1500, 500) + labs(x = "Runtime (ms)", y = "Density", title = "Density Plot")
    ...
    private fun openPlot(plot: Plot, name: String) {
        // Export to HTML.
        // Note: if all you need is to save HTML to a file than you can just use the 'ggsave()'
        // function.
        val content =
            PlotHtmlExport.buildHtmlFromRawSpecs(
                plot.toSpec(), scriptUrl(VersionChecker.letsPlotJsVersion), false)
